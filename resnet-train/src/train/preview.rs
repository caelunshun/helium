use crate::{model::NUM_CLASSES, training_data::augmentation::make_srgb8};
...
static LR_PLOT_FILE: &str = "preview/learning_rate.png";

pub struct Previewer {
    train_data: Mutex<LossData>,
    validation_data: Mutex<LossData>,
    lr_data: Mutex<Vec<f32>>,
...
impl Previewer {
    ...
    pub fn add_learning_rate_iteration(&self, lr: f32) {
        self.lr_data.lock().push(lr);
    }
...
fn run_processor_thread(previewer: &Previewer) {
    loop {
        ...
        rayon::join(
            || {
                let train_data = previewer.train_data.lock().clone();
                let validation_data = previewer.validation_data.lock().clone();
                let lr_data = previewer.lr_data.lock().clone();
                ...
                generate_plot("learning rate", &lr_data, &[], LR_PLOT_FILE).unwrap();
...
impl Example {
    fn write_to_disk(&self) -> anyhow::Result<()> {
        ...
        make_srgb8(&self.image).save(format!("{path}.png"))?;
...
fn generate_plot(
    ...
    let root = BitMapBackend::new(output_path, (WIDTH as u32, 1080)).into_drawing_area();
    root.fill(&WHITE)?;

    let x_len = data_train.len() as f32;
    let v_len = data_valid.len() as f32;

    let data_train = resample_data_points(data_train, WIDTH);
    let data_valid = resample_data_points(data_valid, WIDTH);
    ...
    chart
        .draw_series(LineSeries::new(
            data_train.iter().copied().map(|(i, y)| (i as f32, y)),
            &BLACK,
        ))?
    ...
    chart
        .draw_series(LineSeries::new(
            data_valid
                .iter()
                .copied()
                .map(|(i, y)| (i as f32 / v_len * x_len, y)),
            &RED,
        ))?
...
fn resample_data_points(data: &[f32], target_size: usize) -> Vec<(usize, f32)> {
    if data.is_empty() {
        return Vec::new();
    }

    let scale = data.len() as f64 / target_size as f64;
    let gamma = 0.9f64.powf(scale.recip());

    let mut output = Vec::with_capacity(data.len());
    let mut average = data[0] as f64;
    for val in data.iter().copied() {
        average = gamma * average + (1.0 - gamma) * val as f64;
        let i = output.len();
        output.push((i, average as f32));
    }
