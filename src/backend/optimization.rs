...
pub fn generate_plan<B: Backend>(op_graph: &OpGraph, backend: &B) -> Plan<B> {
...
impl<B: Backend> InstrGraph<B> {
    ...
    pub fn fuse_instrs(&mut self, a: InstrNodeId, b: InstrNodeId) {
        debug_assert!(self.instr_dependents(a).any(|x| x == b));
        debug_assert!(self.instr_dependencies(b).any(|x| x == a));
        debug_assert!(self.get(a).can_fuse_with(self.get(b)));

        let new_instr = self.get(a).fuse_with(self.get(b));
...
fn make_instr_graph<B: Backend>(op_graph: &OpGraph, backend: &B) -> InstrGraph<B> {
...
fn generate_plan_from_graph<B: Backend>(mut graph: InstrGraph<B>) -> Plan<B> {
    ...
    for instr_id in graph.instrs.keys() {
        let indegree = graph.instr_dependents(instr_id).count();
    ...
    while !stack.is_empty() {
        ...
        for &instr_id in &step {
            indegrees[instr_id] = indegrees[instr_id].checked_sub(1).unwrap();
            if indegrees[instr_id] == 0 {
                stack.push(instr_id);
            }
        }
