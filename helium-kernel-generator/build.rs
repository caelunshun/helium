...
fn main() {
    ...
    let mut cuda_tarball = tar::Builder::new(BufWriter::new(
        File::create(out_dir.join("cuda.tar")).unwrap(),
    ));
    cuda_tarball.append_dir_all(".", cuda_include_path).unwrap();
    cuda_tarball.finish().unwrap();

    let mut cutlass_tarball = tar::Builder::new(BufWriter::new(
        File::create(out_dir.join("cutlass.tar")).unwrap(),
    ));
    cutlass_tarball.append_dir_all(".", cutlass_path).unwrap();
    cutlass_tarball.finish().unwrap();
}
